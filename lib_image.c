...
void lib_image_include(elf_State *S) {
	...
	,	VSTR(elf_push_new_string(S,"elf.gfx.load_image"))
	,	VCFN(lib_image_load_image));

	elf_table_set(S->M->globals
	,	VSTR(elf_push_new_string(S,"elf.gfx.image_replace_colors"))
	,	VCFN(lib_image_replace_colors));


	elf_table_set(S->M->globals
	,	VSTR(elf_push_new_string(S,"elf.gfx.image_mask"))
	,	VCFN(lib_image_mask));
}
...
int lib_image_create(elf_State *S) {
	...
	elf_tsets_int(tab,elf_push_new_string(S,"@ptr"),(elf_Int)(img));
	elf_tsets_int(tab,elf_push_new_string(S,"w"),w);
	elf_tsets_int(tab,elf_push_new_string(S,"h"),h);
...
int lib_image_load_image(elf_State *S) {
	...
	elf_Table *tab = elf_new_table(S);
	elf_tsets_int(tab,elf_push_new_string(S,"@ptr"),(elf_Int)(img));
	elf_tsets_int(tab,elf_push_new_string(S,"width"),!img ? 0 : img->w);
	elf_tsets_int(tab,elf_push_new_string(S,"height"),!img ? 0 : img->h);
	elf_push_table(S,tab);
	return 1;
}
